            ...
            if($($JobCheck.findlastsession()).State -eq "Working"){
                ...
                {  
                    ...
                    else
                    {
                        ...
                        {
                            $global:ExitCode=2
                            $global:OutMessageTemp+="CRITICAL - La replication "+$JobCheck.Name+" n a pas ete execute lors des 2 dernieres heures"
                            $global:CriticalCount++
                        }
                        else
                        {
                            ...
                            $global:OutMessageTemp+="execute le "+("{0:D2}" -f $LastRun.Day)+"/"+("{0:D2}" -f $LastRun.Month)+"/"+$LastRun.year+" "+("{0:D2}" -f $LastRun.hour)+":"+("{0:D2}" -f $LastRun.Minute)+" "
                            $global:OkCount++
                        }
                    }
                }
